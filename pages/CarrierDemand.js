  ...
  render() {
    ...
    return (
      <PageContainer {...this.props} bannerenable={true}>
        <NavigationEvents onDidFocus={() => this.getinformation()} />
        <Spinner
            ...
            <TouchableOpacity
              style={
                this.state.tab == 'pending'
                  ? style.tabitemactive
                  : style.tabiteminactive
              }
              onPress={() => this.settab('pending')}>
              <Text
                style={
                  this.state.tab == 'pending'
                    ? style.tabtext
                    : style.tabinactivetext
                }>
                {intlData.messages['PENDIENTE']}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={
            ...
            {this.state.data[this.state.tab].map((row, index) => {
              return (
                <TouchableOpacity
                  ...
                  onPress={() =>
                    this.props.navigation.navigate('MYDEMANDITEM', {id: row.id})
                  }>
                  <View style={{flexDirection: 'row'}}>
                    <View style={{flex: 1}}>
                        ...
                        <Image
                          source={{
                            uri:
                              ...
                              row.pickCountry.toLowerCase() +
                              '/flat/64.png',
                          }}
                          style={style.flagimage}
                        />
                        <Text style={{marginLeft: 10}}>
                          {row.pickCityName ? row.pickCityName : row.pickCity}
                        </Text>
                        <Text> > </Text>
                        <Image
                          source={{
                            uri:
                              'https://www.countryflags.io/' +
                              row.deliverCountry.toLowerCase() +
                              '/flat/64.png',
                          }}
                          style={style.flagimage}
                        />
                      ...
                      <Text>
                        Recogida:{' '}
                        {Moment(new Date(row.pickDayIni)).format('DD-MMM')} -
                        Entrega:{' '}
                        {Moment(new Date(row.deliverDayIni)).format('DD-MMM')}
                      </Text>
                    </View>
                    {row.count > 0 && (
                      <View
                        style={{width: wp('10%'), justifyContent: 'center'}}>
