...
class Proposalitem extends React.Component {
  ...
  getinformation = () => {
    ...
    service.getdemanditem(demandid).then(async result => {
      ...
      let transportista = await service.gettransportista(
        proposal.transportista,
      );
  ...
  sendemail = () => {
    ...
    let self = this;
    service
      .sendmail(this.state.proposal, ask_desc, intlData.messages['ASK_CARRIER'])
      .then(function() {
  ...
  PaymentIOS = async amount => {
      ...
      transactions: [
        {
          ...
          item_list: {
            shipping_address: {
              recipient_name: 'carloslantero@hotmail.com',
              line1: 'VeryHorse',
              line2: Number(amount).toFixed(2),
              city: '-',
              country_code: 'ES',
              postal_code: 'EUR',
              phone: '34670401111',
    ...
    let reqData = {grant_type: 'client_credentials'};
    let body = Object.keys(reqData)
      .map(function(key) {
    ...
    let response = await axios.post(
      ...
      body,
      {
        ...
        auth: {
          username:
            'AVv3vM7y10lkYDGPryyHXvVYCkb_h0LuBl0hJsh7kBz0YNg2HUNpYpbMknhTpWRg2n4PvwW4W7oCFR6R',
          password:
            'EAEJP90gxRH6EITyaizDp2wO8QDyg6R7YhXuMSw1rGm072CEk-byo7D0gK09xnEEDiQKoeZmx43eJh5j',
        },
      },
    );
    this.setState({
    ...
    axios
      .post('https://api.paypal.com/v1/payments/payment', dataDetail, {
        headers: {
    ...
    switch (data[type + 'Mod']) {
      case 'before':
        string =
          ...
          ' ' +
          Moment(new Date(Number(data[type + 'DayIni']))).format('DD-MM-YYYY');
        break;
      case 'after':
        string =
        ...
        break;
      case 'day':
        string = Moment(new Date(Number(data[type + 'DayIni']))).format(
          'DD-MM-YYYY',
        );
        break;
      case 'between':
  ...
  righttext = () => {
    ...
    return (
      <Text style={{marginLeft: wp('2%'), alignItems: 'center'}}>
        <Text style={style.acceptbefore}>{intlData.messages['LEIDO']}</Text>
        <Text style={style.acceptafter}>
          {' '}
          {intlData.messages['CONDICIONES_GENERAL_CONTRATACION']}
        </Text>
      </Text>
    );
  };
  ...
  paymentcompleted = async amountpaid => {
    const {intlData} = this.props;
    let self = this;
    ...
    try {
      let service = new Service();
      await service.savepaymentsuccessdemand(
        demand,
        self.props.navigation.state.params.demandid,
      );

      await service.addpending(demand, {
        transportista: self.state.proposal.transportista,
        transportistaname: self.state.transportista.name_empresa,
      });
  ...
  sendpayment = async () => {
    ...
    let amountpaid = parseFloat(
      Util.payamount(this.state.proposal.amount),
    ).toString();
    let self = this;
    if (Platform.OS === 'ios') {
      await this.PaymentIOS(amountpaid);
    } else {
  ...
  reservar = async () => {
    ...
    if (Object.keys(err).length == 0) {
      ...
      let self = this;
      Strip.createTokenWithCard(carddetail)
        .then(token => {
          console.log(token);
          var payment = {
            token: token.tokenId,
          ...
          service.payforstripe(payment, function(data) {
            if (data.status != 'error') {
              ...
              if (data.code == 'card_declined') {
                Alert.alert(
                  intlData.messages['ERROR_T'],
                  intlData.messages['TARJETA_R'] + ' ' + data.message,
                );
              } else {
                Alert.alert(
                  'Error',
                  intlData.messages['ERROR_PAGO'] + ' ' + data.message,
                );
              }
            }
          });
  ...
  desestimar = () => {
    ...
    let self = this;
    service.desestimar(
      this.props.navigation.state.params.demandid,
      this.props.navigation.state.params.proposalid,
      this.state.data,
      this.state.proposal,
      function() {
  ...
  _onNavigationStateChange = webViewState => {
    ...
    if (webViewState.url.includes('https://example.com/')) {
      ...
      axios
        ...
        .then(response => {
          let amountpaid = parseFloat(
            Util.payamount(response.data.transactions[0].amount.total),
          ).toString();
          this.paymentcompleted(amountpaid);
        })
        .catch(err => {
    ...
    return this.state.approvalUrl ? (
      ...
      <PageContainer {...this.props} backenable={true} bannerenable={true}>
        ...
        <View style={style.container}>
          <View style={style.demandcontent}>
            <View style={style.itemgroup}>
              ...
              {this.state.proposal.altDelDay && (
                <Text style={style.description}>
                  {Moment(
                    new Date(Number(this.state.proposal.altDelDay)),
                  ).format('DD-MM-YYYY')}{' '}
                  - {this.state.data.pickCityName}, {this.state.data.pickCP} (
                  {this.state.data.pickCountryName})
                </Text>
              )}
              {!this.state.proposal.altDelDay && (
            ...
            <View style={style.itemgroup}>
              ...
              {this.state.proposal.altDelDay && (
                <Text style={style.description}>
                  ...
                  - {this.state.data.deliverCityName},{' '}
                  {this.state.data.deliverCP} (
                  {this.state.data.deliverCountryName})
                </Text>
              )}
              {!this.state.proposal.altDelDay && (
            ...
            <View style={style.itemgroup}>
              <Text style={style.label}>
                {intlData.messages['TRATAMIENTO']}
              </Text>
              <Text style={style.description}>
                {this.state.data.specialDesc
                  ? this.state.data.specialDesc
                  : 'No'}
              </Text>
            </View>
            <View style={style.itemgroup}>
              ...
              <Text style={style.description}>
                {Number(this.state.data.lugage)
                  ? this.state.data.lugageDesc
                  : 'No'}
              </Text>
            </View>
          </View>
          <View style={style.proposalcontent}>
            <View style={style.itemgroup}>
              ...
              <Text style={style.description}>
                {Util.demandamount(this.state.proposal.amount)} €
              </Text>
            </View>
            <View style={style.itemgroup}>
              ...
              <Text style={{fontSize: hp('1.9%'), ...style.description}}>
                {intlData.messages['PROPOSAL_DISCLOUSURE']}
              </Text>
            </View>
            <View style={style.itemgroup}>
            ...
            <View style={style.itemgroup}>
              <Text style={style.label}>
                {intlData.messages['VALOR_TRANS2']}
              </Text>
                ...
                {[0, 1, 2, 3, 4].map(function(value, key) {
                  return (
                    <IonIcon
                      name="md-star"
                      style={
                        value < self.state.rating ? style.onicon : style.officon
                      }
                    />
                  );
                })}
              ...
              {this.state.transportista.description && (
                <Text style={style.description}>
                  {intlData.locale == 'en' &&
                  this.state.transportista.description.en
                    ? this.state.transportista.description.en
                    : this.state.transportista.description.es}
                </Text>
              )}
            </View>
            <View style={style.itemgroup}>
              <Text style={style.label}>
                {intlData.messages['ASK_CARRIER']}
              </Text>
              <TextInput
                style={style.ask_input}
                multiline={true}
                numberOfLines={4}
                value={this.state.ask_description}
                onChangeText={text => this.setState({ask_description: text})}
              />
              <View style={style.askbtncontainer}>
                <TouchableOpacity
                  ...
                  <Text style={style.description}>
                    {intlData.messages['Send_Consult']}
                  </Text>
                </TouchableOpacity>
              </View>
            </View>
            {this.state.data.status == 'pending' && (
              <View>
                <View style={{marginTop: hp('1%'), flexDirection: 'row'}}>
                  <CheckBox isChecked={this.state.tos} onClick={this.accept} />
                  <Text style={{marginLeft: wp('2%'), alignItems: 'center'}}>
                ...
                {this.state.error && (
                  <Text style={style.formerror}>
                    {intlData.messages['ACEPTAR_CONDICIONES']}
                  </Text>
                )}
                <View style={{marginTop: hp('3%')}}>
                  <TouchableOpacity
                    style={style.btncontainer}
                    onPress={this.showreservar}>
                    <Text style={{textAlign: 'center', ...style.description}}>
                      {intlData.messages['RESERVAR']}
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={style.btncontainer}
                    ...
                    <Text style={{textAlign: 'center', ...style.description}}>
                      {intlData.messages['RESERVAR_PAYPAL']}
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={style.btncontainer}
                    onPress={this.deleteproposal}>
                    <Text style={{textAlign: 'center', ...style.description}}>
                      {intlData.messages['DESESTIMAR_QUOTE']}
                    </Text>
                  </TouchableOpacity>
        ...
        <Modal
          ...
          modalTitle={
            <ModalTitle
              title={intlData.messages['INFORMACION']}
          ...
          footer={
            <ModalFooter>
              <ModalButton
                ...
                onPress={() => this.setState({strip: false})}
              />
              <ModalButton
                text={intlData.messages['ENVIAR2']}
                style={{backgroundColor: '#dd691b'}}
          ...
          <ModalContent>
            <View style={{padding: wp('3%'), backgroundColor: 'lightgrey'}}>
              <View style={{marginTop: hp('1%')}}>
                <HTML html={intlData.messages['BEFORE_PAYMENT']} />
              </View>
              <View style={{marginTop: hp('1%')}}>
                <Text style={style.formlabel}>
                  {intlData.messages['NUMERO_T']} *
                </Text>
                <TextInput
                  style={style.forminput}
                  value={this.state.card.cc}
                  onChangeText={text => this.handleChange('cc', text)}
                />
              ...
              <View style={{marginTop: hp('1%'), flexDirection: 'row'}}>
                <View style={{flex: 1}}>
                  <Text style={style.formlabel}>
                    {intlData.messages['MES']} (MM)*
                  </Text>
                  <TextInput
                    style={style.forminput}
                    value={this.state.card.mes}
                    onChangeText={text => this.handleChange('mes', text)}
                  />
              ...
              <View style={{marginTop: hp('1%')}}>
                ...
                <TextInput
                  style={style.forminput}
                  value={this.state.card.ccv}
                  onChangeText={text => this.handleChange('ccv', text)}
                />
        ...
        <Modal
          ...
          modalTitle={
            <ModalTitle
              title={intlData.messages['PAGO_REALIZADO']}
              style={style.paymenttitle}
            />
          }
          footer={
          ...
          <View style={{padding: wp('3%'), backgroundColor: 'lightgrey'}}>
            <View style={{marginTop: hp('1%')}}>
              <HTML html={intlData.messages['TEXTO_PAGO_REALIZADO']} />
            </View>
          </View>
        </Modal>
      </PageContainer>
