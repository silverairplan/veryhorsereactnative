...
class RegisterCarrier extends React.Component
{
    ...
    validate = async(step) => {
        ...
        for(let item in user)
        {
            ...
            {
                case 0:
                    if(["name_empresa","numero_identificacion","idioma"].indexOf(item) > -1)
                    {
                        if(!user[item])
                        {
    ...
    render()
    {
        const {intlData} = this.props;
        return (
            <PageContainer {...this.props} title={intlData.messages["REGISTRO_T"]} backenable={true} backfunction={this.state.step == 0?false:this.prev}>
...
export default connect(RegisterCarrier);