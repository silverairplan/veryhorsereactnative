    ...
    return (
      <View style={{flex: 1}}>
        <PageContainer
          {...this.props}
          bannerenable={true}
          title={intlData.messages['HOTELS']}>
          <MapView
            ...
            {Object.keys(this.state.hoteldata).map((value, index) => {
              if (
                this.state.hoteldata[value].five_stars >=
                this.state.possitiverate
              ) {
                return (
                  <Marker
                    key={index}
                    ...
                    coordinate={{
                      latitude: parseFloat(
                        this.state.hoteldata[value].latitude,
                      ),
                      longitude: parseFloat(
                        this.state.hoteldata[value].longitude,
                      ),
                      latitudeDelta: 0,
                      longitudeDelta: 0,
                    }}>
                    <AutoHeightImage
                      source={
                        parseInt(this.state.hoteldata[value].five_stars) > 0
                          ? require('../assets/imgs/icons/map_green.png')
                          : require('../assets/imgs/icons/map_orange.png')
                      }
                      width={20}
                    />
                    <Callout
                      onPress={() =>
                        this.pressed(this.state.hoteldata[value].id)
                      }>
                      <ScrollView style={style.locationcontainer}>
                        <Text style={style.locationtitle}>
                          {this.state.hoteldata[value].name}
                        </Text>
                        <Text style={style.locationdescription}>
                          {this.state.hoteldata[value].address},{' '}
                          {this.state.hoteldata[value].city},{' '}
                          {this.state.hoteldata[value].country_name}
                        </Text>
                        <TouchableHighlight
                          style={style.more_detail}
                          onPress={() => this.pressed()}>
                          <Text style={style.more_text}>
                            {intlData.messages['VIEW_DETAILS']}
                          </Text>
                        </TouchableHighlight>
                      </ScrollView>
                    </Callout>
          ...
          <View style={style.action_btn}>
            <TouchableOpacity
              ...
              <View style={style.hotelcontainer}>
                <Ionicons name="md-list" style={style.icon} />
            ...
            <TouchableOpacity style={style.ratelist} onPress={this.setpositive}>
              <View style={style.hotelcontainer}>
                <Ionicons
                  name="md-star"
                  style={
                    this.state.possitiverate == 0
                      ? style.icon_star_inactive
                      : style.icon_star_active
                  }
                />
                <Ionicons
                  name="md-star"
