...
export default class DataService {
  ...
  getreviewitem = (uid, id) => {
    return new Promise((resolve, reject) => {
      firebase
        .database()
        .ref('/pending_valoraciones/' + uid + '/' + id)
        .on('value', snapshot => {
    ...
    return new Promise((resolve, reject) => {
      AsyncStorage.getItem('user')
        .then(user => {
          ...
          firebase
            .database()
            .ref('/users/' + valoracion.trans)
            .once('value', snapshot => {
              let trans = snapshot.val();
              console.log(trans);
              let senddata = {
                points: userval.rating,
                desc: userval.desc,
                user: user.uid,
                username: user.name + ' ' + user.lastname,
                demandName: valoracion.demandName,
              };
              ...
              firebase
                ...
                .then(function() {
                  axios.get('http://admin.veryhorse.com/php/send.php', {
                    params: {
                      data: JSON.stringify({
                        user: trans.name_empresa,
                        userEmail: trans.email,
                        demandName: valoracion.demandName,
                        lang: trans.idioma,
                      }),
                  ...
                  firebase
                    .database()
                    .ref(
                      '/pending_valoraciones/' + user.uid + '/' + valoracion.id,
                    )
                    .remove();
                  resolve(true);
                })
