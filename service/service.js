...
export default class Service {
  ...
  loadbanner = language => {
    return new Promise((resolve, reject) => {
      axios
        .get(apiurl + '/get-banners', {params: {lang: language}})
        .then(res => {
          let bannerimages = [];
          ...
          for (let item in data) {
            data[item]['image'] =
              apiurl + '/images/banners/' + data[item]['image'];
  ...
  getinfo = (language = 'es') => {
    return new Promise((resolve, reject) => {
      axios
        .get(apiurl + '/get-static-pages', {params: {lang: language}})
        .then(res => {
          let pages = [];
  ...
  getstaticpage = (page, language = 'es') => {
    return new Promise((resolve, reject) => {
      axios
        .get(apiurl + '/get-static-page/' + page, {params: {lang: language}})
        .then(res => {
          resolve(res.data);
        })
  ...
  getdata = (language = 'es') => {
    return new Promise((resolve, reject) => {
      axios
        .get(apiurl + '/get-hotels-list', {params: {lang: language}})
        .then(res => {
      ...
      axios
        ...
        .then(async res => {
          ...
          for (let item in res.data.hotels) {
            res.data.hotels[item].cover_photo =
              apiurl + '/images/hotels/' + res.data.hotels[item].cover_photo;
          ...
          if (locationinfo) {
            for (let item in hoteldata) {
              hoteldata[item].offset = this.getdistance(
                locationinfo,
                hoteldata[item],
              );
              hoteldata[item].distance = hoteldata[item].offset
                ? hoteldata[item].offset + ' km'
                : '';
            }
  ...
  getdistance = (currentlocation, hotel) => {
    ...
    var a =
      ...
      Math.cos(this.getrad(hotel.latitude)) *
        Math.cos(this.getrad(currentlocation.lat)) *
        Math.sin(dLong / 2) *
        Math.sin(dLong / 2);
  ...
  gethoteldetail = (hotelid, language = 'es') => {
    return new Promise((resolve, reject) => {
      axios
        .get(apiurl + '/get-hotel-info', {
          params: {lang: language, id: hotelid},
        })
        .then(res => {
          let hoteldata = res.data.hotel;
          hoteldata['cover_photo'] =
            apiurl + '/images/hotels/' + hoteldata['cover_photo'];
          if (hoteldata['photos']) {
            for (var i in hoteldata['photos']) {
              hoteldata['photos'][i] =
                apiurl + '/images/photos/' + hoteldata['photos'][i];
            }
          }
          if (hoteldata['whatsapp']) {
            hoteldata['whatsapp'] =
              'https://api.whatsapp.com/send?phone=' +
              hoteldata['whatsapp'].replace(/[\D]+/, '');
          }
  ...
  sendemail = (proposal, content, title) => {
    return new Promise((resolve, reject) => {
      firebase
        .database()
        .ref('/users/' + proposal.transportista)
        .on('value', snapshot => {
          ...
          axios({
            ...
            headers: {
              Authorization:
                'Bearer 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',
              'Content-Type': 'application/json',
            },
            data: {
              personalizations: [{to: [{email: 'info@veryhorse.com'}]}],
              from: {email: users.email},
              subject: title,
              content: [
                {
                  type: 'text/html',
                  value:
                    '<html><body><div><strong>User : ' +
                    users.name +
                    '</strong></div><div><strong>User Email : ' +
                    users.email +
                    '</strong></div><div><strong>Price : ' +
                    Util.demandamount(proposal.amount) +
  ...
  sendcontactinfo = (data, language, callback) => {
    data.lang = language;
    axios
      .get('http://admin.veryhorse.com/php/send.php', {
        params: {data: JSON.stringify(data), action: 'send_emergencias'},
      })
      .then(res => {
        callback(res.data);
      });
  ...
  createdemand = (data, callback) => {
    AsyncStorage.getItem('user').then(value => {
      ...
      data.pickDayIni = data.pickDayIni
        ? new Date(data.pickDayIni).getTime()
        : 0;
      data.pickDayEnd = data.pickDayEnd
        ? new Date(data.pickDayIni).getTime()
        : 0;
      data.deliverDayIni = data.deliverDayIni
        ? new Date(data.deliverDayIni).getTime()
        : 0;
      data.deliverDayEnd = data.deliverDayEnd
        ? new Date(data.deliverDayEnd).getTime()
        : 0;
      firebase
        .database()
        .ref('/demandas')
        ...
        .then(function(ref) {
          let senddata = {
            demand: ref.key,
            pickCountry: data.pickCountry,
            deliverCountry: data.deliverCountry,
            pickCity: data.pickCity,
            deliverCity: data.deliverCity,
          };
          axios
            .get('http://admin.veryhorse.com/php/send.php', {
              params: {
        ...
        firebase
          ...
          .on('value', async snapshot => {
            ...
            snapshot.forEach(function(item) {
              ...
              if (
                data.status == status &&
                ((data.pickDayEnd && +new Date(data.pickDayEnd) > today) ||
                  +new Date(data.pickDayIni) > today)
              ) {
                array.push(data);
              }
            });
            ...
            for (let item in array) {
              array[item].count = await this.getdemandquotecount(
                array[item].id,
              );
            }
            resolve(array);
          ...
          snapshot.forEach(value => {
            ...
            if (proposal.transportista == transporterId) {
              ...
              if (proposal.triptype) {
                itemCopy.tripTitle =
                  proposal.triptype === 'one_way_trip'
                    ? 'TRIP_ONE_WAY'
                    : 'TRIP_ROUND';
        ...
        firebase
          ...
          .on('value', async snapshot => {
            ...
            snapshot.forEach(val => {
              ...
              demand.id = val.key;
              if (
                !Number.isNaN(Number(demand.pickDayIni)) &&
                demand.status != 'canceled' &&
                demand.status != 'refund' &&
                (new Date(demand.pickDayIni).getTime() > today ||
                  (demand.pickDayEnd &&
                    new Date(demand.pickDayEnd).getTime() > today))
              ) {
                list.push(demand);
              }
            });
            ...
            for (let item in list) {
              ...
              if (
                !list[item].desestimadas ||
                list[item].desestimadas.indexOf(transporterId) < 0
              ) {
                if (
                  list[item].status == 'confirmed' &&
                  list[item].userTrans == transporterId
                ) {
                  if (
                    (list[item].deliverDayEnd &&
                      list[item].deliverDayEnd >= today) ||
                    (list[item].deliverDayIni &&
                      list[item].deliverDayIni >= today)
                  ) {
                    demands.confirmed.push(list[item]);
                  }
                } else if (
                  list[item].status == 'pending' &&
                  this.isItemInteresting(list[item], user)
                ) {
                  let results = await this.parsedemand(
                    list[item],
                    transporterId,
                  );
  ...
  isItemInteresting = (item, user) => {
    if (!item.desestimadas || item.desestimadas.indexOf(user.uid) < 0) {
      ...
      if (
        user.interestedCountries.indexOf(item.pickCountry) >= 0 ||
        user.interestedCountries.indexOf(item.deliverCountry) >= 0
      ) {
        return true;
      } else {
        var continentPick = this.dataservice.getContinentFromISO(
          item.pickCountry,
        );
        var continentDel = this.dataservice.getContinentFromISO(
          item.deliverCountry,
        );

        if (
          user.interestedCountries.indexOf(continentPick) >= 0 ||
          user.interestedCountries.indexOf(continentDel) >= 0
        ) {
          return true;
        }
      }
  ...
  deletequote = (demandid, proposalid) => {
    return new Promise((resolve, reject) => {
      firebase
        .database()
        .ref('/proposals/' + demandid + '/' + proposalid)
        .remove()
        .then(res => {
          resolve(res);
        });
  ...
  deletedemand = (id, callback) => {
    firebase
      .database()
      .ref('/demandas/' + id + '/status')
      .set('canceled', function(error) {
        callback(true);
      });
  };

  savepaymentsuccessdemand = async (demand, demandid) => {
        ...
        AsyncStorage.getItem('user').then(value => {
          ...
          axios.get('http://admin.veryhorse.com/php/send.php', {
            params: {
              data: JSON.stringify({
                demand: demandid,
                proposal: demand.acceptedProposal,
                transportista: demand.userTrans,
                user: user.uid,
              }),
  ...
  addpending = (demand, data) => {
    return new Promise((resolve, reject) => {
      ...
      firebase
        .database()
        .ref('/pending_valoraciones/' + demand.user)
        .push({
          date: nowsecond,
      ...
      .then(function() {
        axios
          .get('http://admin.veryhorse.com/php/send.php', {
            params: {
              data: JSON.stringify({
                demand: demand.pickCity + ' a ' + demand.deliverCity,
                user: proposal.transportista,
              }),
      ...
      AsyncStorage.getItem('user').then(user => {
        ...
        let senddata = {
          ...
          content:
            ...
            user.email +
            '</strong></div><div><strong>Price : ' +
            parseFloat(Util.demandamount(proposal.amount)) +
            '€ </strong> </div><div><strong>Vehicle : ' +
            proposal.vehicle +
            '</strong></div><div><p>' +
            desc +
        ...
        axios
          .get('http://admin.veryhorse.com/php/send.php', {
            params: {data: JSON.stringify(senddata), action: 'ask_carrier'},
          })
          .then(function() {
            resolve(true);
          });
  ...
  sendproposal = (demandid, demand, data) => {
    return new Promise((resolve, reject) => {
      if (data.id) {
        ...
        firebase
          .database()
          .ref('/proposals/' + demandid + '/' + data.id)
          .set(proposalitem)
          .then(function() {
            resolve(true);
          });
              ...
              .get('http://admin.veryhorse.com/php/send.php', {
                params: {
                  data: JSON.stringify({
                    ...
                    demand: demandid,
                    proposal: data,
                    amount: Util.demandamount(data.amount),
                  }),
      ...
      AsyncStorage.getItem('user').then(value => {
        ...
        if (user.type == 'transportista') {
            ...
            .on('value', async snapshot => {
              ...
              if (snapshot.exists()) {
                snapshot.forEach(route => {
                  ...
                  if (
                    routedata.status == state &&
                    new Date(routedata.pickDayIni).getTime() >
                      new Date(
                        today.getFullYear(),
                        today.getMonth(),
                        today.getDate(),
          ...
          firebase
            .database()
            .ref('/routes')
            .orderByChild('pickDayIni')
            .startAt(Moment(today).format('YYYY-MM-DD'))
            .on('value', snapshot => {
              let list = [];
              if (snapshot.exists()) {
                snapshot.forEach(route => {
        ...
        firebase
          ...
          .on('value', snapshot => {
            ...
            snapshot.forEach(value => {
              ...
              if (
                demand.status != 'canceled' &&
                (demand.user = useritem.uid || useritem.type == 'transportista')
              ) {
                list.push(demand);
              }
            });
